...
###############################################################################
#another possibility when the data has unique observations, with gaussian_kde from scipy.stats
...
###############################################################################
# Q5: Distribution of number of children by family
...
###############################################################################
# Q8: Fit regression model for MeanParents

male = df['Gender'] == 'M'
female = df['Gender'] == 'F'
fig = plt.figure()
plt.plot(df['MeanParents'][male], y[male], 'o', c=my_blue)
plt.plot(df['MeanParents'][female], y[female], 'o', c=my_purple)
plt.plot(X0, skl_lm.predict(X0), label='Linear prediction', c='k')
plt.xlabel('MeanParents', fontsize=18)
...
skl_lm = linear_model.LinearRegression()
skl_lm.fit(df[['Height']], df['MeanParents'])
...
###############################################################################
# 11 OLS with the 2 featurs 'Father', 'Mother'
...
print('With normalization the main effect is due' +
      ' to {}'.format(X2.columns[np.argmax(results_norm)]))
